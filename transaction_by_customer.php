...
if (!is_user_logged_in())
 ...
 global $wpdb;
    ...
    SET image = '%s' WHERE id = '%d' and user_id=%s and company_id=%s", $image,$customer_id,$user_id,$company->company_id));
...
$customer_id=sanitize_text_field($_REQUEST['customer_id']);
 ...
 $customer = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}customer where id=%s and company_id=%s ", $customer_id,$company->company_id));
 
   $invoice = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}invoice where customer_id=%s  and company_id=%s ", $customer_id,$company->company_id));
  $estimate = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}estimate where customer_id=%s   and company_id=%s", $customer_id,$company->company_id));
 
 $totalcredit = $wpdb->get_var($wpdb->prepare("SELECT SUM(amount) as totalamount FROM {$wpdb->prefix}aistore_wallet_transactions where  type = 'credit' and customer=%s  and company_id=%s",$customer->customer_name,$company->company_id));
 
  $totaldebit = $wpdb->get_var($wpdb->prepare("SELECT SUM(amount) as totalamount FROM {$wpdb->prefix}aistore_wallet_transactions where  type = 'debit' and customer=%s  and company_id=%s",$customer->customer_name,$company->company_id));
 
$totalsum=    $totalcredit - $totaldebit  ;
 
$sql = ($wpdb->prepare("SELECT * FROM {$wpdb->prefix}aistore_wallet_transactions where customer=%s and company_id=%s ", $customer->customer_name,$company->company_id));
...
<div class="tab-content" id="myTabContent">
  ...
  Email: <?php echo $customer->customer_email; ?><br><br>
...
<input class="input" type="hidden" id="customer_id" name="customer_id" value="<?php echo $customer_id; ?>"><br><br>
...
?>    <tr>
  ...
  <div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">  <h2>Estimate</h2>
