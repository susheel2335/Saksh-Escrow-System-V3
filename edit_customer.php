...
function aistore_edit_customer(){
    if (!is_user_logged_in())
    {
     ...
     global $wpdb;
      ...
      $company = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}company where user_id=%s ",$user_id));   
       ...
       $customer = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}customer where id=%s and user_id=%s and company_id=%s ", $customer_id,$user_id,$company->company_id));
    ...
    SET customer_name = '%s', customer_email = '%s' , customer_type =%s,mobile_no=%s,first_name=%s,last_name=%s,country=%s,address=%s,city=%s,state=%s ,zip_code=%s, pan_number =%s, gst_number=%s WHERE id = '%d' and user_id=%s", $customer_name,$customer_email,$customer_type,$mobile_no,$first_name,$last_name,$country,$address,$city,$state,$zip_code,$pan_number,$gst_number,$customer_id,$user_id));
...
<label for="Individual">Individual</label><br><br>
  ...
  <input class="input" type="text" id="customer_name" name="customer_name" value="<?php echo $customer->customer_name;?>"><br>


  <label for="title"><?php _e('Email', 'aistore'); ?></label><br>
  <input class="input" type="text" id="customer_email" name="customer_email" value="<?php echo $customer->customer_email;?>"><br><br>
